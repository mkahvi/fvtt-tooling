...
async function buildJS() {
	...
	log.info('JS', pc.bold(path.basename(result.file.main)), pc.dim(`(in ${result.time.total} ms)`));
}

async function buildCSS() {
	const result = await css.build();
	log.info('CSS', pc.bold(path.basename(result.file.main)), pc.dim(`(in ${result.time.total} ms)`));
}
